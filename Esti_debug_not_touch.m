...
for i = 2:dt:(tb*1000+1)
    ...
    P_tank_fuel(i) = P_tank_fuel(i-1).*(V_gas_fuel(i-1) ./ (sum(m_dot_fuel(1:i-1)).*(dt/1000)./rho_f.*9.81 + V_gas_fuel(i-1))).^k;
    P_tank_oxid(i) = P_tank_oxid(i-1).*(V_gas_oxid(i-1) ./ (sum(m_dot_oxid(1:i-1)).*(dt/1000)./rho_ox.*9.81 + V_gas_oxid(i-1))).^k;
