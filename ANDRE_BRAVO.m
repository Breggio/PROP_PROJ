...
[R_cooling_f] = cooling_losses(f, d_pipe, L, rho_f, A_pipe);
[R_cooling_ox] = cooling_losses(f, d_pipe, L, rho_ox, A_pipe);
...
for i = 2:dt:(tb*10+1)
    ...
    P_tank_fuel(i) = Pt_in_f*(V_gas_in_f / ( sum( m_dot_fuel(1:i-1) )*(dt/10)/rho_f*9.81 + V_gas_in_f ));
    P_tank_oxid(i) = Pt_in_ox.*(V_gas_in_ox ./ (sum(m_dot_oxid(1:i-1)).*(dt/10)./rho_ox.*9.81 + V_gas_in_ox));
...
%     [R_valves_ox(i)] = valves_losses(rho_f);
    ...
    R_tot_fuel(i) = R_inj_f(i) + R_feed_f(i) + R_dyn_f(i);
    R_tot_oxid(i) = R_inj_ox(i) + R_feed_ox(i) + R_dyn_ox(i);
    [m_dot_fuel(i)] = mass_flow_rate(P_tank_fuel(i), P_c(i),  R_tot_fuel(i));
    [m_dot_oxid(i)] = mass_flow_rate(P_tank_oxid(i), P_c(i), R_tot_oxid(i));
