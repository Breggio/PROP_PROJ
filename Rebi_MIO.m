...
d_inj_f = 0.0005; % [m] Fuel injector diameter DA VERIFICARE CON ADDITITVE MANUFACTURING

Delta_P_inj = 0.2*Pc_in; %IPOTIZZATA COME PRIMA GUESS

% A_f = m_dot_f/(Cd*sqrt(2*8.9576e+5*rho_f)); % [m^2] Fuel total injection area
...
% A_ox = m_dot_ox/(Cd*sqrt(2*1.7042e+6*rho_ox)); % [m^2] Oxidizer total injection area
...
% u_ox = Cd*sqrt(2*8.9576e+5/rho_ox); % [m/s] Oxidizer discharge velocity
% u_f = Cd*sqrt(2*1.7042e+6/rho_f); % [m/s] Fuel discharge velocity
