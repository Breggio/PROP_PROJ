...
OF = 6; % O/F ratio - from previous computations
...
for i = 2:dt:(tb*1000+1)
    ...
    P_tank_fuel(i) = P_tank_fuel(i-1).*(V_gas_fuel(i-1) ./ (m_dot_fuel(i-1).*(dt/1000)./rho_f.*9.81 + V_gas_fuel(i-1))).^k;
    P_tank_oxid(i) = P_tank_oxid(i-1).*(V_gas_oxid(i-1) ./ (m_dot_oxid(i-1).*(dt/1000)./rho_ox.*9.81 + V_gas_oxid(i-1))).^k;
    ...
    [R_valves_ox(i)] = valves_losses(rho_ox);
    ...
    [m_dot_fuel(i)] = mass_flow_rate(P_tank_fuel(i), P_c(i), R_feed_f(i) + R_dyn_f(i) + R_inj_f(i) + R_valves_f(i));
    [m_dot_oxid(i)] = mass_flow_rate(P_tank_oxid(i), P_c(i), R_feed_ox(i) + R_dyn_ox(i) + R_inj_ox(i) + R_valves_ox(i));
    m_dot_tot(i) = m_dot_oxid(i) + m_dot_fuel(i); 
    O_F(i) = m_dot_oxid(i)/m_dot_fuel(i);
    V_gas_fuel(i) = V_gas_fuel(i-1)*P_tank_fuel(i-1)/P_tank_fuel(i);
    V_gas_oxid(i) = V_gas_oxid(i-1)*P_tank_oxid(i-1)/P_tank_oxid(i);
    i
